...
var require_common = __commonJS((exports, module) => {
  ...
  var is_comment_symbol = (subject) => {
    const key = Symbol.keyFor(subject);
    return is_string(key) && (NON_PROP_SYMBOL_PREFIXES.includes(key) || PROP_SYMBOL_PREFIXES.some((prefix) => key.indexOf(prefix + COLON) === 0));
  };
  var remove_blank_line_tokens = (comments) => {
    let write = 0;
    let removed = false;
  ...
  var remove_blank_lines_from_prop = (target, prop) => {
  ...
  var remove_blank_lines_deep = (target) => {
...
var require_parse = __commonJS((exports, module) => {
  ...
  var create_blank_line = () => ({
    type: "BlankLine",
    inline: false
  });
  var append_blank_lines = (comments, from_line, to_line) => {
  ...
  var normalize_parse_options = (options) => {
  ...
  var parse_comments = (prefix) => {
    const comments = [];
    let previous_line = last ? last.loc.end.line : 0;
    ...
    append_blank_lines(comments, previous_line, current ? current.loc.start.line : source_line_count);
    if (remove_comments) {
...
`;
  ...
  var is_inline_whitespace = (char) => char === SPACE || char === "\t";
  var count_trailing_line_breaks = (str) => {
    let i = str.length;
    let count = 0;
    while (i > 0) {
  ...
  var process_comments = (host, symbol_tag, deeper_gap, display_block) => {
    ...
    if (!last_comment) {
      return blank_lines ? repeat_line_breaks(blank_lines + 1, deeper_gap) : EMPTY;
    }
    const default_line_breaks_after = display_block || last_comment.type === "LineComment" ? 1 : 0;
    const line_breaks_after = blank_lines > 0 ? blank_lines + 1 : default_line_breaks_after;
  ...
  var join2 = (one, two, gap) => one ? two ? one + two.trim() + LF + gap : one.trimRight() + repeat_line_breaks(Math.max(1, count_trailing_line_breaks(one)), gap) : two ? two.trimRight() + repeat_line_breaks(Math.max(1, count_trailing_line_breaks(two)), gap) : EMPTY;
  ...
  var normalize_blank_lines = (str) => typeof str === "string" ? str.replace(/\n[ \t]+(?=\n)/g, `
`) : str;
...
var PLUGIN_ENTRY = "opencode-background-panel";
function isBackgroundPanelEntry(entry) {
  if (!entry)
    return false;
  if (entry === PLUGIN_ENTRY)
    return true;
  if (entry.startsWith(`${PLUGIN_ENTRY}@`))
    return true;
  if (entry.includes("opencode-background-panel"))
    return true;
  return false;
}
function resolveTuiConfigPath() {
  const configDir = join2(homedir2(), ".config", "opencode");
  const jsoncPath = join2(configDir, "tui.jsonc");
  const jsonPath = join2(configDir, "tui.json");
...
function ensureTuiPluginEntry() {
  try {
    ...
    writeFileSync(configPath, `${import_comment_json.stringify(config, null, 2)}
`);
    console.log(`[BTP] updated TUI plugin entry in ${configPath}`);
    return true;
  } catch (error) {
    console.log(`[BTP] failed to update tui.json: ${error instanceof Error ? error.message : String(error)}`);
    return false;
  }
}
...
`;
      try {
        writeFileSync2(CONFIG_FILE2, defaultConfig, "utf-8");
      } catch (e) {
    ...
    const content = readFileSync3(CONFIG_FILE2, "utf-8");
    const cleanContent = content.replace(/\/\/.*$/gm, "").replace(/\/\*[\s\S]*?\*\//g, "");
    const config = JSON.parse(cleanContent);
    ...
    if (!config.data_dir || config.data_dir !== DATA_DIR) {
      try {
        ...
        writeFileSync2(CONFIG_FILE2, newContent, "utf-8");
